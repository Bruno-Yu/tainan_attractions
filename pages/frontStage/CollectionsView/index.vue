<template>
  <main>
    ...
    <div class="grid grid-cols-3 gap-4 my-5">
      <!-- cards -->
      <div v-for=" (item) in cartProducts" :key="item.product_id" class="flex justify-center ">
        <div class="rounded-lg shadow-lg bg-white w-full">
          <a href="#!" data-mdb-ripple="true" data-mdb-ripple-color="light">
            <img class="rounded-t-lg h-40 w-full object-cover" :src="item.product.imageUrl" :alt="item.product_id" />
          </a>
          <div class="p-6">
            <h5 class="text-gray-900 text-xl font-medium mb-2">{{ item.product.title }}</h5>
            <div class="flex justify-end">
              <button type="button"
                class=" inline-block px-6 py-2.5 bg-blue-600 text-white font-medium text-xs leading-tight uppercase rounded shadow-md hover:bg-blue-700 hover:shadow-lg focus:bg-blue-700 focus:shadow-lg focus:outline-none focus:ring-0 active:bg-blue-800 active:shadow-lg transition duration-150 ease-in-out">加入購物車</button>
            </div>
          </div>
        </div>
      </div>
